...
public class RUHungry {
    ...
    public void menu(String inputFile) {
        ...
        int categoriesCount = StdIn.readInt();
        categoryVar = new String[categoriesCount];
        menuVar = new MenuNode[categoriesCount];

        for (int z = 0; z < categoriesCount; z++) {
            StdIn.readLine();
            categoryVar[z] = StdIn.readLine();
            int dishesCount = StdIn.readInt();
            MenuNode headMenuNode = null;
            for (int y = 0; y < dishesCount; y++) {
                StdIn.readLine();
                String dishName = StdIn.readLine();
                int ingredientsCount = StdIn.readInt();

                int[] ingredientIDs = new int[ingredientsCount];
                for (int x = 0; x < ingredientsCount; x++) { ingredientIDs[x] = StdIn.readInt(); }
                MenuNode newMenuNode = new MenuNode(new Dish(categoryVar[z], dishName, ingredientIDs), null);

                newMenuNode.setNextMenuNode(headMenuNode);
                headMenuNode = newMenuNode;
            }
            menuVar[z] = headMenuNode;
        }
    }
    ...
    public StockNode findStockNode (int ingredientID) {
        ...
        if (currentStockNode == null) { return null; }
        while(currentStockNode != null) {
            if (currentStockNode.getIngredient().getID() == ingredientID) { return currentStockNode; }
    ...
    public void updateStock (String ingredientName, int ingredientID, int stockAmountToAdd) {
        ...
        if (ingredientName != null) { searchIngredient = findStockNode(ingredientName).getIngredient(); }
        else if (ingredientID != -1) { searchIngredient = findStockNode(ingredientID).getIngredient(); }
        if (searchIngredient != null) { searchIngredient.updateStockLevel(stockAmountToAdd); }
    }
    ...
    public void updatePriceAndProfit() {
        for (int z = 0; z < menuVar.length; z++) {
            MenuNode currentMenuNode = menuVar[z];
            while(currentMenuNode != null) {
                double totalPrice = 0;
                for (int currentID : currentMenuNode.getDish().getStockID()) {
                    Ingredient searchIngredient = findStockNode(currentID).getIngredient();
                    if (searchIngredient != null) { totalPrice += searchIngredient.getCost(); }
                }
                double finalPrice = totalPrice * 1.2;
                currentMenuNode.getDish().setPriceOfDish(finalPrice);
                currentMenuNode.getDish().setProfit(finalPrice - totalPrice);
    ...
    public void createStockHashTable (String inputFile){
        ...
        while (!StdIn.isEmpty()) {
            ...
            StockNode newNode = new StockNode(new Ingredient(ID, name, stockLevel, cost), null);
    ...
    public void addTransactionNode ( TransactionData data ) { // method adds new transactionNode to the end of LL
        ...
        if (transactionVar == null) { transactionVar = newTransactionNode; }
        else {
            TransactionNode currentTransactionNode = transactionVar;
            while(currentTransactionNode.getNext() != null) { currentTransactionNode = currentTransactionNode.getNext(); }
            currentTransactionNode.setNext(newTransactionNode);
        }
    }
    ...
    public boolean checkDishAvailability (String dishName, int numberOfDishes){
        ...
        for (int ingredientID : menuNode.getDish().getStockID()) {
            StockNode searchStockNode = findStockNode(ingredientID);
            if (searchStockNode == null || searchStockNode.getIngredient().getStockLevel() < numberOfDishes) { return false; }
        }
        return true;
    }
    ...
    public void order (String dishName, int quantity){
        ...
        if (checkDishAvailability(dishName, quantity)) {
            ...
            addTransactionNode(new TransactionData("order", dishName, quantity, dish.getProfit() * quantity, true));
            for (int ingredientID : dish.getStockID()) { updateStock(null, ingredientID, -quantity); }
        }
        else {
            addTransactionNode(new TransactionData("order", dishName, quantity, 0, false));
            MenuNode searchMenuNode = menuVar[findCategoryIndex(menuNode.getDish().getCategory())];
            menuNode = menuNode.getNextMenuNode();
            while(true) {
                if(menuNode == null) {
                    if(searchMenuNode == null) { break; }
                    menuNode = searchMenuNode;
                }
                Dish currentDish = menuNode.getDish();
                menuNode = menuNode.getNextMenuNode();
                if(currentDish.getName().equals(dishName)) { break; }
                if (checkDishAvailability(currentDish.getName(), quantity)) {
                    addTransactionNode(new TransactionData("order", currentDish.getName(), quantity, currentDish.getProfit() * quantity, true));
                    for (int ingredientID : currentDish.getStockID()) { updateStock(null, ingredientID, -quantity); }
                    return;
                }
                else { addTransactionNode(new TransactionData("order", currentDish.getName(), quantity, 0, false)); }
            }
        }
    }
    ...
    public void donation (String ingredientName, int quantity){
        Ingredient searchIngredient = findStockNode(ingredientName).getIngredient();
        boolean checkStockForDonation = (searchIngredient != null && searchIngredient.getStockLevel() >= quantity);
        if (profit() > 50 && checkStockForDonation) {
    ...
    public void restock (String ingredientName, int quantity){
        ...
        if (searchIngredient != null) {
            double totalRestockCost = searchIngredient.getCost() * quantity;
            if (profit() >= totalRestockCost) {
                updateStock(ingredientName, -1, quantity);
                addTransactionNode(new TransactionData("restock", ingredientName, quantity, -1 * totalRestockCost, true));
            }
            else { addTransactionNode(new TransactionData("restock", ingredientName, quantity, 0, false)); }
        }
    }
    ...
    */
    ...
    public void seatAllGuests ( Queue<Party> waitingQueue ) {
        leftQueueVar = new Queue<>();
        while (!waitingQueue.isEmpty()) {
            Party nextParty = waitingQueue.dequeue();
            boolean isSeated = false;
            for (int z = 0; z < tables.length; z++) {
                if (tables[z] == null && tableSeats[z] >= nextParty.getNumberOfPeople()) {
                    tables[z] = nextParty;
                    nextParty.setTableIndex(z);
                    isSeated = true;
                    break;
                }
            }
            ...
            if (!isSeated) { leftQueueVar.enqueue(nextParty); }
        }
        for (Party table : tables) {
            if (table != null) { leftQueueVar.enqueue(table); }
        }
        tables = new Party[tableSeats.length];
    }
